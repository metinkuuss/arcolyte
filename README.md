...
A permissionless AMM (automated market maker) built on [Arc Testnet](https://www.arc.network), the EVM-compatible L1 by Circle where USDC is the native gas token.

**Live app:** https://arcolyte.netlify.app

## What it does

Arcolyte lets anyone launch a token, open a trading pool for it, and swap — all in one place, without writing any code.
...
### Zap
Don't hold the paired token? Deposit USDC only. Arcolyte automatically swaps half of it for the paired token and deposits both sides as balanced liquidity, in a single transaction.
...
- Frontend: vanilla JavaScript + [ethers.js](https://docs.ethers.org/), no backend, no database — everything reads and writes directly on-chain
- Hosted on Netlify
